...
class MessagePumpKqueue : public MessagePump {
 ...
 private:
  ...
  nsTHashMap<uint32_t, FileDescriptorWatcher*> fd_controllers_;
  uint32_t next_fd_controller_id_ = 0;

  // Watch controllers for Mach ports. IDs are the port being watched.
  nsTHashMap<uint32_t, MachPortWatchController*> port_controllers_;
