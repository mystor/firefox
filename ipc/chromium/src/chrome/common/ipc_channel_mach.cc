...
bool ChannelMach::ContinueConnect(mozilla::UniqueMachSendRight send_port) {
  ...
  // Now that we have a send port, request that the kernel notify us when this
  // port dies.
  ...
  if (kr != KERN_SUCCESS) {
    ...
    CHROMIUM_LOG(ERROR) << "mach_port_request_notification failed: "
                        << mach_error_string(kr);
    return false;
  }
...
template <typename T>
static void CleanupNotificationPort(mach_msg_header_t* header,
                                    mach_port_t expected_port) {
  MOZ_RELEASE_ASSERT(header->msgh_size ==
                     sizeof(T) - sizeof(std::declval<T>().trailer));
  auto* notification = reinterpret_cast<T*>(header);
  MOZ_RELEASE_ASSERT(notification->not_port == expected_port);

  // MACH_NOTIFY_DEAD_NAME and MACH_NOTIFY_SEND_POSSIBLE notifications sent by
  // the kernel are somewhat poorly behaved, in that they contain references to
  // mach ports which are not automatically cleaned up by `mach_msg_destroy`, so
  // we need to do this manually.
  (void)mach_port_deallocate(mach_task_self(), notification->not_port);
  notification->not_port = MACH_PORT_NULL;
}
  ...
  *header = mach_msg_header_t{.msgh_size = Channel::kReadBufferSize,
  ...
  if (SenderIs(trailer, KERNEL_AUDIT_TOKEN_VALUE)) {
    if (header->msgh_id == MACH_NOTIFY_DEAD_NAME) {
      CleanupNotificationPort<mach_dead_name_notification_t>(header,
                                                             send_port_.get());
      // The other side has gone away, close the channel.
      return false;
    }
    if (header->msgh_id == MACH_NOTIFY_SEND_POSSIBLE) {
      CleanupNotificationPort<mach_send_possible_notification_t>(
          header, send_port_.get());
      mozilla::MutexAutoLock lock(SendMutex());
      return ProcessOutgoingMessages();
    }
    return true;
  ...
  if (!msg_body || (msg_body->msgh_descriptor_count > 0) !=
  ...
  while (!output_queue_.IsEmpty()) {
    ...
    if (kr == KERN_SUCCESS) {
      ...
      if (kr == MACH_SEND_TIMED_OUT) {
        // Set ourselves up to be called back when sending becomes possible.
        mozilla::UniqueMachSendRight previous;
        kr = mach_port_request_notification(
            mach_task_self(), send_port_.get(), MACH_NOTIFY_SEND_POSSIBLE, 0,
            receive_port_.get(), MACH_MSG_TYPE_MAKE_SEND_ONCE,
